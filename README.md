...
- transient, namespaced Pexip application messages share countdowns and successful Start/Pause notices with the other connected chairs;
...
This is not a single button that changes mode. Keeping both controls visible makes the available action explicit and allows any chair to pause a hearing without relying on local state from the chair who started it. During a local Start, its configured countdown, or a Pause operation, both controls are temporarily disabled to prevent overlapping moves. They become available again when the operation and any required hold have completed.
...
- Starting broadcasts a namespaced Pexip application message so every connected chair running the host-only plugin sees the same configured, non-dismissible countdown in their own selected language. No chat message is created.
